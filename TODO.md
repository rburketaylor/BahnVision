...
- Implement station search and route lookup endpoints so the backend exposes MVG features beyond departures. Reference the `mvg` nearby/lines APIs. (design-doc.md:21-23, backend/README.md:49)
- Add deterministic cache-aside configuration with per-endpoint TTL controls to match the production-grade caching roadmap. (backend/README.md:66-72)
- Introduce stampede protection (single-flight locks) plus soft-TTL background refresh for cached departures. (backend/README.md:70-75)
...
- Instrument cache and MVG client interactions with metrics/logging for hit/miss ratios, fetch latency, and error counts. (backend/README.md:78)

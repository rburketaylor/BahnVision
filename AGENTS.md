...
- Root `design-doc.md` captures the system overview; `docker-compose.yml` wires the backend and Redis.
...
- `pip install -r backend/requirements.txt` — install FastAPI, MVG client, and Redis dependencies.
- `uvicorn app.main:app --reload --app-dir backend/app` — start the API with hot reload at `http://127.0.0.1:8000`.
- `docker compose up --build` — launch the backend plus Redis using the compose file; persists local cache data in the container network.
...
- Test FastAPI routes via the `TestClient` and mock Redis interactions to keep suites deterministic.
...
- Store secrets (Redis URLs, API tokens) in environment variables or `.env` files that stay out of version control.

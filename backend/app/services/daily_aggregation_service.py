...
class DailyAggregationService:
    ...
    def __init__(
        ...
        Args:
            ...
            source_bucket_width_minutes: Bucket width for source hourly stats
        """
    ...
    async def aggregate_day(self, target_date: date) -> int:
        ...
        transaction_ctx: AbstractAsyncContextManager = (
        ...
        async with transaction_ctx:
            ...
            hourly_stmt = (
                select(
                    ...
                    func.coalesce(
                        func.sum(RealtimeStationStats.on_time_count), 0
                    ).label("on_time_count"),
                    func.coalesce(
...
def should_use_daily_summary(from_time: datetime, to_time: datetime) -> bool:
    ...
    """
    threshold_seconds = _DAILY_SUMMARY_THRESHOLD_DAYS * _SECONDS_PER_DAY
    return (to_time - from_time).total_seconds() >= threshold_seconds

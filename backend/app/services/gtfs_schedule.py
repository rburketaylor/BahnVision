...
class GTFSScheduleService:
    ...
    async def get_stop_departures(
        ...
        # Pre-calculate midnight once to avoid doing it for every row in interval_to_datetime
...
def interval_to_datetime(base_datetime: datetime, interval_value) -> Optional[datetime]:
    """Convert PostgreSQL interval to a concrete UTC datetime from a base datetime.

    Args:
        base_datetime: The starting point (usually midnight of the service date in UTC).
        interval_value: The interval to add (timedelta or string representation).

    Handles GTFS times that extend beyond 24h by adding the full timedelta to
    the base datetime instead of wrapping to a time-of-day.
    """

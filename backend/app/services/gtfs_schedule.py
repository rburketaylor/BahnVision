...
class GTFSScheduleService:
    ...
    async def get_stop_departures(
        ...
        """
        # Step 1: Resolve relevant stops (target + children)
        # This replaces the inefficient OR join in the main query
        stops_query = select(GTFSStop.stop_id, GTFSStop.stop_name).where(
            or_(GTFSStop.stop_id == stop_id, GTFSStop.parent_station == stop_id)
        )
        stops_result = await self.session.execute(stops_query)

...
class TestGetStopDepartures:
    ...
    def _mock_active_services(self, active_ids=None, added_ids=None, removed_ids=None):
        ...
        # Combine into rows: (service_id, exception_type)
        rows = []
        for sid in active_ids:
            rows.append((sid, 0))  # 0 for calendar active
        for sid in added_ids:
            rows.append((sid, 1))
        for sid in removed_ids:
            rows.append((sid, 2))

        # Result for the UNION query
        mock_union = MagicMock()
        mock_union.all = MagicMock(return_value=rows)
    ...
    async def test_get_stop_departures_supports_calendar_dates_only(
        ...
        mock_union = self._mock_active_services(
            active_ids=[], added_ids=["service_dates_only"]
        )

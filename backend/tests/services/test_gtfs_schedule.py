...
class TestGetStopDepartures:
    ...
    def _mock_active_services(self, active_ids=None):
        ...
        # Return mock rows containing (service_id, exception_type)
        # where exception_type 0 means active calendar entry
        mock_rows = [MagicMock(service_id=sid, exception_type=0) for sid in active_ids]

        mock_union = MagicMock()
        mock_union.all = MagicMock(return_value=mock_rows)
    ...
    async def test_get_stop_departures_weekday_service(self, service, mock_session):
        ...
        mock_union = self._mock_active_services()
        ...
        mock_session.execute = AsyncMock(
            side_effect=[mock_stop_result, mock_union, mock_departure_result]
        )
    ...
    async def test_get_stop_departures_supports_calendar_dates_only(
        ...
        mock_union.all = MagicMock(return_value=[mock_row])

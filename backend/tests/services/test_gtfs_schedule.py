...
class TestGetStopDepartures:
    ...
    def _mock_active_services(self, active_ids=None):
        """Create mock results for get_active_service_ids query."""
        if active_ids is None:
            active_ids = ["service_1"]

        # Create mock rows for union query result
        # Default active services have exception_type=0
        mock_rows = []
        for svc_id in active_ids:
            row = MagicMock()
            row.service_id = svc_id

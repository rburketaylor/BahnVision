...
class TestGTFSScheduleService:
    ...
    @pytest.mark.asyncio
    async def test_get_stop_departures_stop_not_found_with_validation_true(
        self, service, mock_session
    ):
        """Test that StopNotFoundError is raised for unknown stops (default behavior)."""
        ...
        with pytest.raises(StopNotFoundError, match="not found"):
            await service.get_stop_departures(
                "unknown_stop", datetime.now(timezone.utc), validate_existence=True
            )

    @pytest.mark.asyncio
    async def test_get_stop_departures_stop_not_found_with_validation_false(
        self, service, mock_session
    ):
        """Test that unknown stops return empty list when validation disabled."""
        # Mock departures query to return empty list
    ...
    async def test_get_departures_for_stop_alias(self, service):
        ...
        with patch.object(
            ...
            await service.get_departures_for_stop(
                "stop1", now, 10, validate_existence=False
            )
...
class TestGetStopDepartures:
    ...
    async def test_get_stop_departures_weekday_service(self, service, mock_session):
        ...
        departures = await service.get_stop_departures(
            "de:09162:6", query_time, limit=10, validate_existence=False
        )
    ...
    async def test_get_stop_departures_supports_calendar_dates_only(
        ...
        # First execute() call is the departures query
    ...
    async def test_get_stop_departures_includes_parent_station_children(
        ...
        query_time = datetime(2025, 12, 8, 8, 0, tzinfo=timezone.utc)  # Monday
        await service.get_stop_departures(
            "parent_station_id", query_time, limit=10, validate_existence=False
        )
    ...
    async def test_get_stop_departures_respects_limit(self, service, mock_session):
        ...
        query_time = datetime(2025, 12, 8, 8, 0, tzinfo=timezone.utc)
        departures = await service.get_stop_departures(
            "de:09162:6", query_time, limit=5, validate_existence=False
        )

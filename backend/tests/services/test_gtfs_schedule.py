...
class TestGetStopDepartures:
    ...
    def _mock_active_services(self, active_ids=None):
        ...
        # Mock result for UNION ALL query
        # Returns (service_id, exception_type) tuples
        # exception_type 0 = calendar active
        rows = [(sid, 0) for sid in active_ids]
    ...
    async def test_get_stop_departures_weekday_service(self, service, mock_session):
        ...
        mock_services = self._mock_active_services()
        ...
        mock_session.execute = AsyncMock(
            side_effect=[mock_stop_result, mock_services, mock_departure_result]
        )
    ...
    async def test_get_stop_departures_supports_calendar_dates_only(
        ...
        # 1 means added exception
        rows = [("service_dates_only", 1)]
        mock_services = MagicMock()
        mock_services.all = MagicMock(return_value=rows)

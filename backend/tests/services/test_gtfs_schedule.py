...
class TestGTFSScheduleService:
    ...
    async def test_get_stop_departures_stop_not_found(self, service, mock_session):
        ...
        # Mock stop query to return empty list
...
class TestGetStopDepartures:
    ...
    async def test_get_stop_departures_includes_parent_station_children(
        ...
        # Verify first query (stop resolution) checks parent_station
        first_query_obj = mock_session.execute.call_args_list[0][0][0]
        first_sql = str(first_query_obj)
        assert "parent_station =" in first_sql

        # Verify second query (departures) uses IN clause
        query_obj = mock_session.execute.call_args_list[1][0][0]
        sql = str(query_obj)
